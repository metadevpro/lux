...
describe('Modal Tests', () => {
    ...
    it('should resolve result promise on close', () => {
        // Arrange
        let resolvedResult;
        component.openModal1().result.then((result) => { resolvedResult = result; });
        // Act
        spectator.detectChanges();
        const button = document.querySelector('button#close') as HTMLElement;
        button.click();
        ...
        const modals = document.querySelector('body').querySelectorAll('.lux-modal-body');
        expect(modals.length).toBe(0);
        spectator.fixture.whenStable().then(() => { expect(resolvedResult).toBe('Close click'); });
    });

    it('should reject result promise on close', () => {
        // Arrange
        let rejectedResult;
        component.openModal1().result.catch((result) => { rejectedResult = result; });
        // Act
        spectator.detectChanges();
        const button = document.querySelector('button#dismiss') as HTMLElement;
        button.click();
        ...
        const modals = document.querySelector('body').querySelectorAll('.lux-modal-body');
        expect(modals.length).toBe(0);
        spectator.fixture.whenStable().then(() => { expect(rejectedResult).toBe('Dismiss click'); });
    });
